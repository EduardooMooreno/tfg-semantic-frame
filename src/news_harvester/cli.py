    ...
    while current_date <= end_dt:
        ...
        try:
            ...
            if daily_articles:
                ...
                filtered = []
                for a in daily_articles:
                    host = urlparse(str(a.url)).netloc.lower()
                    host = host.removeprefix("www.")
                    domain = (a.domain or "").strip().lower().removeprefix("www.")

                    # bloquea .cat por host o por domain de GDELT
                    if host.endswith(".cat") or domain.endswith(".cat"):
                        continue

                    filtered.append(a)

                daily_articles = filtered
                ...
                for article in daily_articles:
                    if str(article.url).lower().find(".cat") != -1:
                        raise RuntimeError(f"ME ESTOY COMIENDO UN .CAT y NO LO FILTRO: {article.url}")
                    domain = (article.domain or "").strip().lower()
                    if domain.endswith(".cat"):
                        metrics["n_skipped_no_text"] += 1
                        print(f"SKIP domain={domain}", flush=True)  # DEBUG opcional
                        continue
                    ...
                    news = (getattr(record, "newspaper", "") or "").strip().lower()
                    news = news.removeprefix("www.")

                    # ✅ filtro duro por dominio catalán / ediciones english
                    if news.endswith(".cat") or news.startswith("english."):
                        metrics["n_skipped_no_text"] += 1  # o crea un metric nuevo si quieres
                        print(f"SKIP newspaper={news}", flush=True)  # <-- DEBUG para ver que funciona
                        continue
                ...
                if daily_records:
                    ...
                    # ✅ FILTRO DEFINITIVO: fuera cualquier .cat (por newspaper y por url)
                    df["newspaper"] = df["newspaper"].astype(str).str.lower().str.replace("^www\.", "", regex=True)
                    df["url"] = df["url"].astype(str)

                    df = df[~df["newspaper"].str.endswith(".cat", na=False)]
                    df = df[~df["url"].str.contains(r"://(www\.)?[^/]*\.cat(/|$)", regex=True, na=False)]
