...
#https://api.gdeltproject.org/api/v2/doc/doc?query=israel%20sourcelang:SPANISH%20sourcecountry:SP&mode=ArtList&maxrecords=250&format=json&startdatetime=20231010000000&enddatetime=20231110235959
#el que no: https://api.gdeltproject.org/api/v2/doc/doc?query=israel%20sourcelang:SPANISH%20sourcecountry:SP&mode=ArtList&maxrecords=250&format=json&startdatetime=20231010000000&enddatetime=20231011000000
...
def fetch_articles(
    *,
    ...
    query_parts.append("sourcelang:SPANISH")
    ...
    logger.info("GDELT query: %s", query)
    ...
    try:
        while True:
            ...
            from curl_cffi import requests as cffi_requests  # ya lo tienes

            max_tries = 6
            for attempt in range(max_tries):
                try:
                    # timeout por request: (connect, read) si curl_cffi lo acepta; si no, usa float
                    response = client_obj.get(
                        GDELT_BASE_URL,
                        params=req_params,
                        timeout=timeout,  # o timeout=(15.0, 180.0) si quieres “connect/read”
                    )
                except Exception as exc:
                    wait = min(60, 2 ** attempt)
                    logger.warning("Request falló (%s). Sleeping %ss (attempt %s/%s)", exc, wait, attempt+1, max_tries)
                    time.sleep(wait)
                    continue

                if response.status_code == 429:
                    wait = min(60, 2 ** attempt)
                    logger.warning("429 Too Many Requests. Sleeping %ss (attempt %s/%s)", wait, attempt+1, max_tries)
                    time.sleep(wait)
                    continue
                ...
                raise GDELTError("GDELT sigue fallando tras varios reintentos (timeout/429/otros).")

            try:
                text = response.text
                # a veces GDELT devuelve basura extra o se corta; intentamos rescatar el JSON
                start = text.find("{")
                end = text.rfind("}")
            ...
            for item in batch:
                ...
                if domains_set and article.domain.lower() not in domains_set:
                    continue
                if article.language and article.language.upper() != "SPANISH":
                    continue
...
def _try_wayback_machine(client: cffi_requests.Session, article: Article) -> str | None:
    ...
    try:
        ...
        if closest and closest.get("available"):
            ...
            wb_resp = client.get(snapshot_url, timeout=120.0)
