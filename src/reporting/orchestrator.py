...
class Phase4Orchestrator:
    ...
    def generate_reports(self, phase3_csv_path, output_dir_base):
        ...
        def _safe_path(p):
            # Python en Windows acepta C:/Users/... y evita unicodeescape (\U, \t, etc.)
            return str(p).replace("\\", "/")

        # Prepare replacements
        # We need to wrap paths in quotes because they will be injected directly into code
        # Prepare replacements with SAFE defaults (can be overwritten below)
        
        replacements = {
            "PHASE3_CSV": f"'{_safe_path(phase3_csv_path)}'",
            "CSV_PATH": f"'{_safe_path(phase3_csv_path)}'",
            "base_dir": f"'{_safe_path(subspaces_dir)}'",
            "ARTIFACTS_DIR": f"'{_safe_path(artifacts_dir)}'",
            "ANCHORS_CSV": "''",
            "DIMENSIONS_JSON": "''",
            "PROJECT_ROOT": f"'{_safe_path(self.project_root)}'",
        }
        ...
        for d in possible_anchors_dirs:
            ...
            for name in possible_anchors_names:
                ...
                if os.path.exists(p):
                    replacements["ANCHORS_CSV"] = f"'{_safe_path(p)}'"
        ...
        for d in possible_dims_dirs:
            ...
            for name in possible_dims:
                p = os.path.join(d, name)
                if os.path.exists(p):
                    replacements["DIMENSIONS_JSON"] = f"'{_safe_path(p)}'"
        ...
        if os.path.exists(manifest_path):
            replacements["MANIFEST_JSON"] = f"'{_safe_path(manifest_path)}'"
        ...
        replacements["sys.path.append('..')"] = f"sys.path.append('{_safe_path(self.project_root)}')"
