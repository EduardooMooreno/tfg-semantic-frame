...
def plot_projection_comparison(df, metric_prefix='score_centroid_', title_prefix='Proyección', output_path=None):
    ...
    """
    setup_pub_style()
    dims = ['violence_security', 'humanitarian_civilian', 'legitimacy_responsibility']
    colors = {'violence_security': '#004e66','humanitarian_civilian': '#5d7667','legitimacy_responsibility': '#d14a2b'}
...
def plot_flexible_projection(df, anchors_path=None, variant='baseline', strategy='penultimate', 
    ...
    # 1. Resolve directories explicitly
    base_phase3_dir = r'C:\Users\emore\TFG\LISBETH-main\LISBETH-main\data\Phase3_Final'
    anchors_dir = os.path.join(base_phase3_dir, 'artifacts', 'anchors')
    subspaces_dir = os.path.join(base_phase3_dir, 'artifacts', 'subspaces')
    ...
    print(f"Using anchors: {anchors_path}")

    # 4. Load anchors
    try:
        with np.load(anchors_path) as data:
            anchors_mat = data['A']  # Shape (D, 3)
            anchor_dims = ['violence_security', 'humanitarian_civilian', 'legitimacy_responsibility']
    ...
    # 6. Iterate over windows and load subspaces
    for idx, row in df.iterrows():
        ...
        for cand in candidates:
            full_path = os.path.join(subspaces_dir, cand)
        ...
        # 8. Compute cosine similarity with each anchor axis
    ...
    # 9. Build plot dataframe
    if len(dates) == 0:
        print("No valid projections could be computed.")
        return

    plot_df = pd.DataFrame(results, index=dates)
    plot_df.index.name = 'date'

    # 10. Plot
    plt.figure(figsize=(10, 6))
    for dim in anchor_dims:
        plt.plot(plot_df.index, plot_df[dim], marker='o', label=dim)

    plt.axhline(0, color='gray', linestyle='--', linewidth=1)
    plt.xticks(rotation=45)
    plt.ylabel('Projection (cosine similarity)')
    plt.xlabel('Time window')

    if title_prefix is None:
        title_prefix = f"Projection ({target_dimension})"

    plt.title(f"{title_prefix} | {variant} | {strategy} | {condition}")
    plt.legend()
    plt.tight_layout()
