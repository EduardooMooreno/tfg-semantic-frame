...
class KSelector:
    ...
    def run(self, Xc: np.ndarray, B_HORN: int = 200, B_BOOT: int = 200, seed: int = 42) -> tuple[int, int, int]:
        ...
        for _ in range(B_BOOT):
            ...
            try:
                _, s_boot, _ = la.svd(X_boot.astype(np.float64), full_matrices=False, lapack_driver="gesvd")
            except Exception:
                # Fallback ultra robusto: pequeño jitter numérico
                X_boot = X_boot.astype(np.float64) + 1e-10 * np.random.default_rng(seed).standard_normal(X_boot.shape)
                _, s_boot, _ = la.svd(X_boot, full_matrices=False, lapack_driver="gesvd")
